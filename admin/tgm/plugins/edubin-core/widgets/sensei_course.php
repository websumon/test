 ...
 */
add_action('widgets_init', 'pxcv_sensei_course_load_widgets');

function pxcv_sensei_course_load_widgets()
{
    register_widget('PXCV_Sensei_Course_Widget');
}
 ...
 *
 */
class PXCV_Sensei_Course_Widget extends WP_Widget
{
    /**
     * Widget setup.
     */
    function __construct()
    {
        ...
        $widget_ops = [
            ...
            'description' => esc_html__('Display Sensei recent course by categories (or related)', 'edubin-core')
        ];

        /* Create the widget. */
        parent::__construct('pxcv-sensei-course', esc_html__('(Edubin) Sensei Courses', 'edubin-core'), $widget_ops);
    }
                            ...
                            if($price_data == '-'): 
                                echo '<span class="course-price">';
                                    echo '<span class="price">';  
                                        echo esc_html($value_of_course);  
                                    echo '</span>';
                                echo '</span>';
                            else :  ?>
                                <a class="price__link" href="<?php the_permalink(); ?>"><?php esc_html__('See More', 'edubin-core'); ?></a>
                            <?php endif;

                        endif; //End show price
    ...
    function form($instance)
    {
        ...
        $defaults = [
            'title' => esc_html__('Sensei Courses' , 'edubin-core'),
            'num_posts' => 3,
            'categories' => 'all',
            'show_related'  => 'off',
            'show_image' => 'on',
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php esc_html_e('Filter Category:' , 'edubin-core'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
                ...
                <?php $categories = get_categories( 'hide_empty=0&depth=1&type=course&taxonomy=course_category' ); ?>
